...
|-------------|-----------------------------------------------|----------------------------------------------------------|----------------------------------------------------------|
| **Администратор** | `$thisstaff->isAdmin() === true`       | Все сотрудники и отделы, полный экспорт                  | Бейдж 🔴, **блок «Администрирование»** внизу (журнал + триггер) |
| **Руководитель**  | `$thisstaff->isManager() === true`     | Все сотрудники и отделы, доступна фильтрация             | Бейдж 🟠, селекты «Сотрудник» / «Отдел» активны          |
| **Агент**         | По умолчанию                           | Только тикеты, назначенные на него                       | Бейдж 🟢, селекты фильтра скрыты, подсказка о scope'e    |
